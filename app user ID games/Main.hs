...
main = do
    ...
    gameStatesVar <- newTVarIO initialGameStates
...
eventHandler :: TVar (Map.Map UserId GameState) -> Event -> DiscordHandler ()
      ...
      case user of 
        ...
        (Left guildMemb) -> do
          ...
          case interactUser of 
            Nothing -> pure ()
            Just id' -> do
              case getGameState (userId id') currentGameStates of
                Nothing -> pure ()
                Just game@(board,_) -> do
                  case move of 
                    Nothing -> do
                      case componentDataCustomId click of
                        "ttt restart" -> do
                          liftIO $ atomically $ modifyTVar' gameStatesVar (updateGameState (userId id') newGame)
                            ...
                            if isWinner botBoard X || isWinner botBoard O
                              then editIntercation interactionId interactionToken (if botPlayer == O then ":x: wins" else ":o: wins") afterBotMove True disableAllButtons 3 3
                                ...
                                then editIntercation interactionId interactionToken "Draw" afterBotMove True disableAllButtons 3 3
                                  ...
                                  liftIO $ atomically $ modifyTVar' gameStatesVar (updateGameState (userId id') afterBotMove)
                ...
                Nothing -> do
                  void $ restCall (R.CreateMessageDetailed (messageChannelId m) (
                    def { 
                          ...
                          R.messageDetailedComponents = Just $ boardToActionRows newGame 3 3 ++ [restartButton]
                        }))
                  updateMapState m gameStatesVar newGame
                  pure ()
                Just game@(_,player) -> do
                  void $ restCall (R.CreateMessageDetailed (messageChannelId m) (
                    def { 
                          R.messageDetailedContent = (if player == X then ":x: to move" else ":o: to move"),
                          R.messageDetailedComponents = Just $ disableUserButtons $ boardToActionRows game 6 7 ++ [restartButton]
                        }))
                  pure ()
            "!r" -> do
              sendMessages m ["restarting game...", packBoard newGame]
              updateMapState m gameStatesVar newGame
